...
def sample(memory, batch_size):
    ...
    states = torch.stack(
        [F.one_hot(torch.tensor(s.board), 4).permute(2, 0, 1).float() for s in states]
    ).to(device)
    next_states = torch.stack(
        [
            F.one_hot(torch.tensor(s.board), 4).permute(2, 0, 1).float()
            ...
            if j % 32 == 0 and len(mem_buffer) >= 1000:
                ...
                with torch.no_grad():
                    ...
                    q_target = torch.where(
                        dones.bool(), rewards, rewards + 0.99 * q_next
                    )
